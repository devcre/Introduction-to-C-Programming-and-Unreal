...
//		this.searchPage = searchPage;
		...
		setContentPane(main = new JPanel(card = new CardLayout(10, 10)));
	...
	void infoUI() {
		...
		var s = new JPanel(new GridLayout(1, 0, 10, 10));
		...
		cn.setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 10));
		c.add(cn, "North");
		c.add(cc);
		c.add(cs, "South");

		s.add(btn("닫기", Color.ORANGE, e -> dispose()));
		s.add(btn("예약하기", Color.ORANGE, e -> card.show(main, "reserve")));

		cn.add(lbl("사진", 2, 15));
		cn.add(new JLabel(getIcon(r.get(8), 370, 150)));
		cn.add(lbl("정보", 2, 15));
		cn.add(Box.createRigidArea(new Dimension(0, 15)));
		...
		tmp2.add(btn(rateNo.isEmpty() ? "후기 작성하기" : "후기 수정하기", Color.ORANGE, e -> {
		...
		tmp2.add(btn("삭제", Color.ORANGE, e -> {
	...
	void reUI() {
		...
		s.add(btn("뒤로가기", Color.ORANGE, e -> card.show(main, "info")));
		s.add(btn("예약하기", Color.ORANGE, e -> {
			...
			if (LocalDate.parse(txt[2].getText(), DateTimeFormatter.ISO_DATE).isBefore(LocalDate.now())) {
				eMsg("예약날짜를 확인해주세요.");
				return;
			}
		...
		// JCombobox에 들어갈 데이터 채우기
		for (var i = start; i.isBefore(end.minusMinutes(30)); i = i.plusMinutes(30)) {

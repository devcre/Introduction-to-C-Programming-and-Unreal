...
public class AdminPage extends BasePage {
	public AdminPage() {
		add(w = sz(new JPanel(new FlowLayout(0)), 200, 140), "West"); // 왼쪽 파란색 관리자 페이지
		...
		String str = "<html>&#128100회원관리,<html>&#127968건물관리,<html>&#128200통계,<html>&#128275로그아웃";
		
		for (var cap : str.split(",")) {
			var lbl = sz(hyplbl(cap , 2, 20, Color.orange, e -> {
				...
				l.setBorder(new CompoundBorder(new MatteBorder(0, 3, 0, 0, Color.orange), new EmptyBorder(0, 5, 0, 0))); // 선택바
	...
	class User extends JPanel { // 회원정보들
		...
		public User() {
			...
			for (var cap : "수정,삭제".split(",")) {
				s.add(btn(cap, Color.ORANGE,  nula -> {
					if (cap.equals("수정")) {

...
public class SignPage extends BasePage {
	...
	public SignPage() {
		...
		add(c = sz(new JPanel(new GridLayout(0, 1)), 250, 500));
...
//		});
		...
		c.add(btn("회원가입", Color.ORANGE, a -> {
			...
			Stream.of(txt).filter(t -> Arrays.asList(txt).indexOf(t) != 3).map(JTextField::getText).forEach(data::add);
			data.add(com.getSelectedIndex() + 1);
			execute("insert user values(0,?,?,?,?,?,?)", data.toArray());
			String sqlString = "FLUSH PRIVILEGES";
			execute(sqlString);
			iMsg("회원가입이 완료되었습니다.");

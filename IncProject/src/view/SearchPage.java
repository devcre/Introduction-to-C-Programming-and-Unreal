...
public class SearchPage extends BasePage {
	...
	public SearchPage() {
		...
		add(west = new West(), "West"); // A 영역
		add(map = new Map());           // B 영역
	...
	class West extends JPanel {
		...
		public West() {
			...
			s.add(hyplbl("메인으로", 2, 1, 15, Color.orange, e -> mf.swapPage(new MainPage(user))));

			nc.add(searchTxt = new JTextField());
			nc.add(btn("검색", Color.ORANGE, e -> search()), "East");
			...
			pathPanelN.add(btn("↑↓", Color.ORANGE, e -> {
			...
			pathPanelNs.add(btn("집을 출발지로", Color.ORANGE, e -> {
				departTxt
		...
		void search() {
			...
			var rs = getRows(
					...
					"%" + searchTxt.getText() + "%");
			if (rs.isEmpty() || rs.get(0).toString().contains("도로")) {
				iMsg("검색 결과가 없습니다.");
				return;
			}
	...
	class Map extends JPanel {
		...
		public Map() {
			...
			var ma = new MouseAdapter() {
				...
				@Override
				public void mouseWheelMoved(MouseWheelEvent e) { // 마우스 휠 움직일 때

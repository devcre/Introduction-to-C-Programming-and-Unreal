...
public interface Tool {
	...
	default JLabel lbl(String c, Color col, int a, int st, int sz) {
		var l = new JLabel(c, a);
		l.setForeground(col);
		l.setFont(new Font("맑은 고딕", st, sz));
		return l;
	...
	default JLabel lbl(String c, Color col, int a, int sz) {
		return lbl(c, col, a, 1, sz);
	}
	...
	default JLabel hyplbl(String c, int a, int st, int sz, Color col, Invoker i) {
		var l = lbl(c, a, st, sz);
		l.setForeground(col);
		l.addMouseListener(new MouseAdapter() {
			@Override
	...
	default JButton btn(String c, Color col, ActionListener a) {
		var b = new JButton(c);
		b.setBackground(col);
		b.addActionListener(a);
